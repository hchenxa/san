...
- **随处运行** —— 单一约 12 MB 二进制，零运行时依赖（无需 Node.js、Python）。原生 Go：冷启动约 0.01s、基线内存约 32 MB，同一文件可在笔记本、边缘设备或 `scratch` 容器中直接运行。提供 Windows、macOS、Linux 三平台构建产物（[体积](docs/operations/footprint.md) · [基准测试](#基准测试san-vs-claude-code)）。
- **权限系统** —— 基于模式的访问控制：询问（默认）与自动接受，`Shift+Tab` 切换。Subagent 继承权限门控，保证沙箱化执行（[详情](docs/concepts/permission-model.md)）。
- **事件驱动协同** —— 基于 pub/sub hub 的并行 subagent 执行（[架构说明](docs/packages/subagent.md)）。
- **会话持久化** —— 自动保存、恢复（`--continue`、`--resume`）、Fork（`/fork`）以及自动上下文压缩（`/compact`）。
- **成本追踪** —— 跨所有提供商的逐消息、逐会话 token 费用统计。
- **主题与外观** —— 通过 `/config` Appearance 面板切换 TUI 配色；"auto" 主题自动匹配终端外观。
...
| 切换权限模式 | `Shift+Tab`（询问 · 自动接受） |
| 搜索 / 人设 / 记忆 | `/search` · `/identity` · `/memory` |
